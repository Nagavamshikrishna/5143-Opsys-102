...
# Define the name of the history table and its columns
table_name = "command_history"

# Define the columns for the command history table
columns = ["id INTEGER PRIMARY KEY", "commands TEXT NOT NULL"]
unique_constraint = "UNIQUE (id)"

# Drop the command history table if it exists
conn.drop_table(table_name)

# Create the command history table with the specified schema
conn.create_table(table_name, columns, unique_constraint)
